...
qcbsmdc_get_tables <- function(conn,form_id,table='all')
{
  if (table=='all') {
    ...
    for (tab in table_list) {
      tt<-getForm('http://quebio.ca/odk/api/download_submission.php',user_id=conn$user_id,api_key=conn$api_key,formId=form_id,table=tab)
      tables[[tab]]<-read.csv(textConnection(tt))
    }  
  }else{
     tt<-getForm('http://quebio.ca/odk/api/download_submission.php',user_id=conn$user_id,api_key=conn$api_key,formId=form_id,table=table)
